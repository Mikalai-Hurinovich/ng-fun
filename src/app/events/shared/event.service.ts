...
export class EventService {
  ...
  searchSessions(searchTerm: string): Observable<ISession[]> {
    return this.http.get<Array<ISession>>(`/api/sessions/search?search=${searchTerm}`)
      .pipe(catchError(this.handleError<ISession[]>('search')));
    // const term = searchTerm.toLocaleLowerCase();
    // let results: Array<ISession> = [];
    // EVENTS.forEach(e => {
    //   let matchingSessions = e.sessions.filter(session => session.name.toLocaleLowerCase().indexOf(term) > -1);
